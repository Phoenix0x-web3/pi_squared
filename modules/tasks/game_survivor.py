...
class GameSurvivor(BaseHttpClient):
    ...
    async def make_random_actions(self, start_time: float, move_x_start=False, move_y_start=False, completed_actions={}):
        ...
        action = await self.complete_action(action_name=action)
        if not action:
            sleep_time = start_time + 63 - time()
            logger.info(f"{self.user} get 429 error. Sleep {sleep_time} and send end game request!")
            await asyncio.sleep(sleep_time)
            completed_actions["Pickup"] = random.randint(50, 70)
