...
class WalletClient:
    ...
    async def get_nonce(self):
        success, data = await self.auth_client.request(url=f"{self.BASE_LINK}/auth/fastset/nonce", use_refresh_token=False, method="GET")
        logger.debug(data)
    ...
    async def connect_wallet(self):
        ...
        success, data = await self.auth_client.request(
            url=f"{self.BASE_LINK}/auth/fastset/link", json_data=json_data, use_refresh_token=False, method="POST"
        )
    ...
    async def send_tokens(self):
        ...
        send = await self.fastset_client.transactions.send_token_transfer(
            recipient_address_set=self.fastset_client.account.address, amount=amount
        )
