...
class ReactorGeometry:
    ...
    @property
    def arena_radius_px(self) -> float:
        return self.container_px * 0.46

    @property
    def orb_hit_r_px(self) -> int:
        return self.orb_size_px // 2

    @property
    def omega_deg_per_sec(self) -> float:
        return 360.0 / self.rotation_period_s  # 9°/s
...
class FrontAccurateOrbArena:
    ...
    def color_center_now(self, color: str) -> Tuple[int, int]:
        ...
        return (int(round(self.cx + self.g.orbit_radius_px * math.cos(ang))), int(round(self.cy + self.g.orbit_radius_px * math.sin(ang))))
...
class StaticArenaOverlay:
    ...
    def draw_static(self, *, target_color: str, ms_left: int):
        ...
        for c in self.arena.colors:
            ...
            for dr in range(-r_cells_y, r_cells_y + 1):
                rr = orow + dr
                if not (0 <= rr < self.arena.h):
                    continue
                for dc in range(-r_cells_x, r_cells_x + 1):
                    cc = ocol + dc
                    if not (0 <= cc < self.arena.w):
                        continue
